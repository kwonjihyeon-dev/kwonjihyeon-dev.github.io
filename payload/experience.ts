  ...
  list: [
    {
      ...
      positions: [
        ...
        },
        {
          ...
          descriptions: [
            '레거시(PHP/Laravel) 페이지를 Next.js(SSR/RSC)로 이관 → **TTFB 950ms → 200ms(-79%), LCP 3.5s → 2.3s, 용량 4.1MB → 1.7MB(-59%)** 개선',
            '기능을 바꿀 때마다 변경된 코드 외에 기존 동작에서 문제가 생기거나 운영 CS가 인입되던 상황을 방지하고자 **핵심 플로우(매물 등록 · 광고 전송)를 자동 검증하는 Playwright 기반 E2E QA 자동화 환경을 구축**',
            '코드 변경에 맞춰 직접 테스트를 갱신하지 않으면 검증이 누락되는 문제를 해소하고자, **LLM이 연관 코드의 diff를 분석해 테스트 갱신 · 추가하고 PR까지 생성**하는 파이프라인을 구성',
        ...
        },
        {
          ...
          descriptions: [
            '모바일웹에서는 앱 설치를 유도하고, 관심 매물 관리 화면을 앱 전용 기능으로 새로 개발해 **App 진입률 62.59%, 모바일웹 → 앱 설치 전환율 5.69% 증가**에 기여',
        ...
        },
        {
          ...
          descriptions: [
            '6개 프로젝트가 사용하는 토큰 값을 각자 하드코딩 · 개별 관리해 토큰 변경 시 수동 동기화와 불일치가 반복되던 문제를 해결하고자, Figma 토큰을 @tokens-studio/sd-transforms + Style Dictionary로 변환해 환경별 출력물(TailwindCSS, SCSS)을 자동 생성하는 파이프라인 구축',
            'Figma 토큰 변경만으로 6개 프로젝트에 일괄 반영되어, 수동 동기화 작업과 불일치 제거',
